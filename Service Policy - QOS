...
from SIFRE import username, password
...
def policy():
    ...
    DC_IP = "192.168.10.1" # A device that is connected via SSH
    ...
    try:
        ...
        LOGfile = "log_gebze_dc1.txt" # as we received unexpected characters in "result3", its been added into a log file. 
        log = open(LOGfile, 'w')
        log.write(result3)
        log.close()
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        with open("log_gebze_dc1.txt", "r") as f: 
            lines6 = f.readlines()
            lines5 = "".join(lines6)
            lines7= lines5.strip("")
            lines = lines7.split()
